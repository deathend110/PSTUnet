...
# AutoDL/vGPU 环境下，NCCL P2P/IB 经常不是稳定路径；launcher 里也会显式设置。
...
def setup_logger(log_dir, is_main_process_flag, rank=0):
    ...
    # 主进程写 train.log；非主进程也写独立 rank 日志，便于定位 DDP 中只有某个 rank 崩溃的问题。
    log_name = "train.log" if is_main_process_flag else f"train_rank{rank}.log"
...
def init_runtime(args):
    ...
    if distributed:
        ...
        # 关键修复：先绑定当前 rank 对应的 CUDA device，再初始化 NCCL。
        # 否则两个 rank 可能先在默认 cuda:0 上触发 NCCL/CUDA 上下文，AutoDL vGPU 环境中容易 SIGSEGV。
...
def main():
    ...
    logger = setup_logger(args.outputs_dir, is_main_process(), get_rank())
    logger.info("=" * 50)
    ...
    if distributed:
        ...
        logger.info("[stage] DDP wrapper ready.")
    elif len(device_ids) > 1:
        logger.info("[stage] Wrap model with DataParallel...")
        wrapped_model = DataParallel(
            wrapped_model,
            device_ids=device_ids,
            output_device=device_ids[0]
        )
        logger.info("[stage] DataParallel wrapper ready.")
    ...
    # 使用纯余弦退火学习率：
    # - 起点仍然是 --lr
    # - 终点 eta_min 固定为 1e-6，保持与原先后期小学习率量级接近
    # - T_max 直接绑定总 epoch 数，因此改 run_autodl_ddp.sh 的 NUM_EPOCHS 会同步改变余弦周期长度
    base_lr = args.lr
    ...
    for epoch in range(args.num_epochs):
        ...
        # 当前学习率在本轮训练开始前读取，和 optimizer 实际使用的值保持一致
        ...
        if is_main_process():
            ...
            progress.set_description(
                f"epoch: {epoch}/{args.num_epochs - 1} [Cosine LR={current_lr:.2e}]"
            )
        ...
        if is_main_process():
            ...
            te = time.time()
            logger.info(
                "Epoch [{}/{}] | scheduler: cosine | lr: {:.2e} | train loss: {:.6f} | eval psnr: {:.4f} | eval ssim: {:.4f} | psnr_score: {:.4f} | Time: {:.2f}s".format(
                    epoch,
                    args.num_epochs - 1,
                    current_lr,
                    train_loss_avg,
